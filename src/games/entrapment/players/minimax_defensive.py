...
class MinimaxEntrapmentPlayerDefensive(EntrapmentPlayer):
    ...
    def __heuristic(self, state: EntrapmentState):
        grid = state.get_grid()
        longest = 0
    ...
    def minimax(self, state: EntrapmentState, depth: int, alpha: int = -math.inf, beta: int = math.inf,
                is_initial_node: bool = True):
        # first we check if we are in a terminal node (victory, draw or loose)
        if state.is_finished():
            return {
                EntrapmentResult.WIN: 40,
                EntrapmentResult.LOOSE: -40,
                EntrapmentResult.DRAW: 0
        ...
        if not is_initial_node:
            ...
            if state.get_result(self.get_opponent_pos()) == EntrapmentResult.WIN:
                return -40  # Strongly discourage opponent's winning moves
